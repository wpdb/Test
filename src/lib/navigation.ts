...
export async function getMenuItems(): Promise<MenuItem[]> {
  ...
  if (config && config.useCustomMenu) {
    if (config.autoSync) {
      const pages = await getNavigationPages();
      const customLinks = config.items.filter((item) => item.type === 'custom');
      const pageOrders = config.pageOrders || {};
      let defaultOrder = 1;

      const autoItems: MenuItem[] = [
        ...(config.showHome && !config.excludedPages.includes('/')
      ...
      return autoItems
        .map((item) => {
          const orderKey = item.type === 'custom' ? 'custom_' + item.id : item.url;
          const fallbackOrder = defaultOrder++;
          return { item, order: pageOrders[orderKey] ?? fallbackOrder };
        })
